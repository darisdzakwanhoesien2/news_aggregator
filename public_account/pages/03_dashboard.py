...
with st.container():
    ...
    if user_df.empty:
        ...
        if "pct_verified" in chart_df:
            st.line_chart(chart_df["pct_verified"])
        if "total_final_score" in chart_df:
            st.bar_chart(chart_df["total_final_score"])
        st.divider()
        st.subheader("Session table")
        cols = ["_session","session_id","company","pct_verified","total_final_score","model","timestamp"]
        present = [c for c in cols if c in user_df.columns]
