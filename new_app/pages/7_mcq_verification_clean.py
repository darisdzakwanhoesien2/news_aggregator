...
# ══════════════════════════════════════════════════════════════════════════════
# Load ESG MCQ questions from JSON file (new_app/data/esg_mcq.json). If the file
# is missing or invalid, fall back to an in-file default (keeps behaviour stable).
# This allows editing the question set without changing the Python code.
ESG_MCQ_JSON = DATA_DIR / "esg_mcq.json"

def _load_esg_mcq() -> list[dict]:
    if ESG_MCQ_JSON.exists():
        try:
            return json.loads(ESG_MCQ_JSON.read_text(encoding="utf-8"))
        except Exception:
            pass

    # Fallback: minimal in-file default if JSON fails (keep at least an empty list)
    return []
...
with tab_questions:
    ...
    # Company selection / creation first (new flow)
    companies = sorted([d.name for d in DATA_DIR.iterdir() if d.is_dir() and d.name != "_tmp"]) if DATA_DIR.exists() else []
    col1, col2 = st.columns([2, 3])
    with col1:
        existing = st.selectbox("Select existing company", ["(new)"] + companies, key="q_select_company")
    with col2:
        if existing == "(new)":
            new_name = st.text_input("Create new company (folder name)", placeholder="e.g. GreenTech_Sdn_Bhd", key="q_new_company")
            company_chosen = new_name.strip() or None
        else:
            company_chosen = existing

    if not company_chosen:
        st.info("Create or select a company to enable question attachment and uploads.")
        st.stop()
    ...
    # Source mode: use existing docs, upload per-question, or upload at end (general attachments)
    source_mode = st.radio("Document source mode", ["Use existing docs", "Upload per-question", "Upload at end (general)"], horizontal=True, key="q_source_mode")

    # Gather existing OCR doc folders for attachments
    ocr_docs = [d.name for d in sorted(ocr_root.iterdir()) if d.is_dir()]

    # If upload-at-end mode, show an uploader to add general docs saved under 'ocr/general'
    if source_mode == "Upload at end (general)":
        with st.expander("📤 Upload general documents (these will be available to attach)"):
            general_uploads = st.file_uploader("Upload files (PDF, TXT, MD, images)", type=["pdf", "txt", "md", "png", "jpg", "jpeg"], accept_multiple_files=True, key="q_general_uploads")
            if general_uploads:
                gen_dir = ocr_root / f"general_{datetime.utcnow().strftime('%Y%m%dT%H%M%SZ')}"
                ...
                for f in general_uploads:
                    ...
                    if (f.type and f.type.startswith("text")) or f.name.lower().endswith(('.txt', '.md')):
                        ...
                        try:
                            (gen_dir / "pages" / "page_001.md").write_text(f.getvalue().decode("utf-8"), encoding="utf-8")
                        except Exception:
                            pass
                st.success(f"Saved {len(general_uploads)} file(s) to data/{company_slug}/ocr/{gen_dir.name}")
                ocr_docs = [d.name for d in sorted(ocr_root.iterdir()) if d.is_dir()]

    # If upload per-question mode, we'll allow a small uploader inside each question expander

    mode = st.radio("Mode", ["View only", "Interactive Questionnaire"], horizontal=True, key="q_mode_main")

    if mode == "Interactive Questionnaire":
        st.info("Answer questions; attach documents based on the selected source mode.")
        chosen_qids = st.multiselect("Questions to include", options=[q['id'] for q in ESG_MCQ], default=[q['id'] for q in ESG_MCQ], key="q_choose_ids")

        with st.form(key="mcq_interactive_form_v2", clear_on_submit=False):
            answers_ui = []
            for q in [qq for qq in ESG_MCQ if qq['id'] in chosen_qids]:
                qid = q['id']
                # Use a simple heading + columns inside the form (avoid nested expanders)
            ...
            submitted = st.form_submit_button("💾 Save answers for company")
            if submitted:
                ts = datetime.utcnow().strftime("%Y%m%dT%H%M%SZ")
                out_dir = DATA_DIR / company_slug / 'mcq_answers'
                out_dir.mkdir(parents=True, exist_ok=True)
                out_file = out_dir / f"{ts}_manual.json"
                record = {'company': company_slug, 'timestamp': datetime.utcnow().isoformat() + 'Z', 'mode': 'manual', 'source_mode': source_mode, 'answers': answers_ui}
                atomic_write_json(out_file, record)
                st.success(f"Saved manual answers → data/{company_slug}/mcq_answers/{out_file.name}")
                st.json(record)
    ...
    st.caption(f"Total: {len(ESG_MCQ)} questions · E: {sum(1 for q in ESG_MCQ if q['pillar']=='Environmental')} · S: {sum(1 for q in ESG_MCQ if q['pillar']=='Social')} · G: {sum(1 for q in ESG_MCQ if q['pillar']=='Governance')}")
